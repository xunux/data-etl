package com.haozhuo.bigdata.dataetl.streamsyn.hive;
...
public class ReportCatalog implements Serializable {
    ...
    public static void insert(Report[] reports) {
        if (reports.length == 0)
            return;
        logger.info("{}个reports开始插入Hive", reports.length);
        ...
        List<String> reportStrs = new ArrayList<String>(reports.length);
        ...
        logger.info("reports插入Hive完成", reports.length);
    }
}
